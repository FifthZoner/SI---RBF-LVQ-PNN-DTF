...
def wynikZDrzewa(drzewo, wektor):
    if drzewo.prawdziwa == None and drzewo.falszywa == None:
        return [drzewo.nrCechy, 1 - drzewo.gini]
...
def runDTF(wejscie, wyniki, tablicaDrzew, maxWysokoscDrzewa, wejscieKontrolne, wyjscieKontrolne):
    ...
    wynikiWiekszosci = [[0 for m in range(len(cechyWgIndeksow))] for n in range(len(wejscieKontrolne))]
    for m in wynikiDrzew:
        for n in range(len(m)):
            wynikiWiekszosci[n][m[n][0]] += m[n][1]
    ...
    for n in range(len(wyjscieKontrolne)):
        max = 0
        for m in range(1, len(wynikiWiekszosci[n])):
            if wynikiWiekszosci[n][m] > max:
                max = m
        if cechyWgIndeksow[max] == wyjscieKontrolne[n]:
            wynikKonsensusu += 1
    for n in range(len(wynikiSuma)):
        print("Celność wytrenowanego nr: ", n + 1, " drzewa to: ", wynikiSuma[n] / len(wyjscieKontrolne) * 100, "%")
    print("Celność lasu drzew z wagą + losowanie przy wielu to: ", wynikKonsensusu / len(wyjscieKontrolne) * 100, "%")
